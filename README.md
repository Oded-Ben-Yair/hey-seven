...
```bash
make test-ci       # ~2537 tests collected, no API key needed (run for exact count)
make test-eval     # 14 live eval tests (requires GOOGLE_API_KEY)
make lint          # ruff + mypy
```

**~2537 tests collected** across 71 test files and 5 layers:
...
├── tests/                        # ~2537 tests across 71 files
