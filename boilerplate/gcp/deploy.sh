...
run_cmd gcloud run deploy "${SERVICE_NAME}" \
    ...
    --timeout=300s \
    --concurrency=80 \
    --set-env-vars="APP_VERSION=${IMAGE_TAG},USE_FIRESTORE=true,GCP_PROJECT_ID=${PROJECT_ID}" \
...
if [ "$DRY_RUN" = false ]; then
    ...
    echo "[DRY RUN] Would verify deployment at Cloud Run URL."
fi
