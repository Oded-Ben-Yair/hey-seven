...
for vector similarity search.  Provides the same interface shape as
``CasinoKnowledgeRetriever`` for swappability in ``get_retriever()``.
...
class FirestoreRetriever:
    ...
    Same interface as ``CasinoKnowledgeRetriever`` for swappability.
    ...
    def retrieve_with_scores(
        ...
        """Retrieve documents with relevance scores via Firestore vector search.

        Embeds the query, calls ``find_nearest()`` with COSINE distance,
        and filters by ``property_id`` metadata to prevent cross-tenant
        leakage.
        ...
        try:
            ...
            logger.info(
                "Firestore vector search returned %d results (query: %.40s...)",
                len(results),
                query,
            )
            return results
