...
class TestInjectGuestName:
    ...
    def test_no_name_returns_unchanged(self):
        assert (
            _inject_guest_name("Welcome to the resort.", None)
            == "Welcome to the resort."
        )

    def test_empty_name_returns_unchanged(self):
        assert (
            _inject_guest_name("Welcome to the resort.", "") == "Welcome to the resort."
        )
    ...
    def test_generic_sentence_lowercases_first_char(self):
        content = (
            "We have several excellent dining options available for you this evening."
        )
        result = _inject_guest_name(content, "Sarah")
        assert (
            result
            == "Sarah, we have several excellent dining options available for you this evening."
        )
    ...
    def test_proper_noun_preserves_case(self):
        ...
        assert (
            result
            == "Sarah, Mohegan Sun has wonderful dining options for your party tonight."
        )

    def test_proper_noun_bobby_flay(self):
        content = (
            "Bobby Flay's Bar Americain is an excellent choice for American grill."
        )
        result = _inject_guest_name(content, "John")
        assert (
            result
            == "John, Bobby Flay's Bar Americain is an excellent choice for American grill."
        )
...
class TestEnforceBranding:
    ...
    def test_exclamation_limit_default(self):
        ...
        result = _enforce_branding(
            content, {"exclamation_limit": 1, "emoji_allowed": False}
        )
    ...
    def test_exclamation_limit_two(self):
        ...
        result = _enforce_branding(
            content, {"exclamation_limit": 2, "emoji_allowed": False}
        )
    ...
    def test_emoji_removed_when_not_allowed(self):
        ...
        result = _enforce_branding(
            content, {"exclamation_limit": 5, "emoji_allowed": False}
        )
    ...
    def test_emoji_kept_when_allowed(self):
        content = "Welcome! 🎰"
        result = _enforce_branding(
            content, {"exclamation_limit": 5, "emoji_allowed": True}
        )
        assert "🎰" in result
